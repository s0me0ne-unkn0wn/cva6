...
SB=$((MB)); for e in "$PROG" "${EXTRA[@]}"; do [ -f "$e.pvmi" ] || ./build_user.sh "$e" $(printf 0x%x $SB) 0 >/dev/null; SB=$((SB+SLOT_SIZE)); done   # placeholders for pass 1
...
for e in "$PROG" "${EXTRA[@]}"; do
  ./build_user.sh "$e" $(printf 0x%x $SB) "$JB" | tail -1
