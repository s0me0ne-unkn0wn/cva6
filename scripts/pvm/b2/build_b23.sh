#!/bin/bash
# build_b23.sh <init_program> [more programs...] [0xMEMORY_BASE]   (2nd program is exposed as /prog2)
...
EXTRA=()                     # further programs land at /<name> in the initramfs (B2.4 exec chaining)
...
for e in "$PROG" "${EXTRA[@]}"; do [ -f "$e.pvmi" ] || ./build_user.sh "$e" "$MB" 0 >/dev/null; done   # placeholders for pass 1
bash ./build_b2.sh "$SPEC" >/dev/null; grep -q "LINK_RC=0" $OUT/build_b2.log || { echo "kernel build 1 failed (see $OUT/build_b2.log)"; exit 1; }
K=$(kval); echo "K = $K"
for e in "$PROG" "${EXTRA[@]}"; do ./build_user.sh "$e" "$MB" "$K" | tail -1; done
