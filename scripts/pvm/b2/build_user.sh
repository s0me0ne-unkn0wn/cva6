#!/bin/bash
# build_user.sh <name> [memory_base] [jt_index_base]
# Rebuild <name>.{polkavm,pvmi} from <name>.S for the cva6-pvm guest kernel (binfmt_pvm).
#  * --emit-relocs: polkatool needs the relocation for the export's metadata pointer
#  * --build-id=none: polkatool rejects .note.gnu.build-id
#  * -mno-relax/--no-relax: linker relaxation leaves relocations polkatool cannot apply ("failed to add
#    addend to the symbol's offset due to overflow") -- same rule as the kernel build
#  * --opt-level 0: O>=1 deletes `t0 = <syscall nr>` before the ecalli (t0 is not an ecalli input
#    register in the metadata, so the optimizer thinks it is dead) -> the kernel would see nr=garbage
#  * memory_base (B2.2, default 0x10000 = code-only programs): link the ro/rw data at the guest
#    kernel's reserved user-data window [0x700000,0x800000) and record it in the PVMI v2 header.
set -eu
cd "$(dirname "$0")"
NAME=${1:?name}; MB=${2:-}; K=${3:-}
P=${POLKATOOL:-/home/claude/pvm/polkavm/target/release/polkatool}
MBARG=(); [ -n "$MB" ] && MBARG=(--memory-base "$MB")
KARG=();  [ -n "$K" ]  && KARG=(--jt-index-base "$K")     # B2.3: K = kernel JT entry count (CFG2[63:40])
CF="-march=rv64emac_zbb -mabi=lp64e -mcmodel=medany -mno-relax -Wl,--no-relax -nostdlib -nostartfiles -static -Wl,--emit-relocs -Wl,--build-id=none -Wno-deprecated"
if [ -f "$NAME.c" ]; then
  # C program: -O2 code + the common start_user.S (metadata, _start -> call main -> exit)
  riscv64-linux-gnu-gcc $CF -O2 -ffreestanding -fno-builtin -fno-jump-tables -o "$NAME.elf" start_user.S "$NAME.c"
else
  riscv64-linux-gnu-gcc $CF -o "$NAME.elf" "$NAME.S"
fi
"$P" link --instruction-set jam_v1 --opt-level 0 --strip "${MBARG[@]}" "${KARG[@]}" -o "$NAME.polkavm" "$NAME.elf"
"$P" emit-image "${MBARG[@]}" "${KARG[@]}" -o "$NAME.pvmi" "$NAME.polkavm"
rm -f "$NAME.elf"
ls -la "$NAME.pvmi"
